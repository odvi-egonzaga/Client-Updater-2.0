...
async function enableRLS() {
  ...
  for (const table of tables) {
    try {
      ...
      if (err.message.includes('does not exist') || err.message.includes('already enabled')) {
        console.log(`  ⊘ Skipped: ${table} (${err.message.includes('does not exist') ? 'table does not exist' : 'RLS already enabled'})`)
        skipped++
      } else {
