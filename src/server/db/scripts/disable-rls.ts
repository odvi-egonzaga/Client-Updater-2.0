...
async function disableRLS() {
  ...
  for (const table of tables) {
    try {
      ...
      if (err.message.includes('does not exist') || err.message.includes('already disabled')) {
        console.log(`  ⊘ Skipped: ${table} (${err.message.includes('does not exist') ? 'table does not exist' : 'RLS already disabled'})`)
        skipped++
      } else {
