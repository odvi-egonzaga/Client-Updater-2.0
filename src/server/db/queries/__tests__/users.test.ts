...
describe("User Queries", () => {
  ...
  describe("getUserWithPermissions", () => {
    it("should return user with permissions, areas, and branches", async () => {
      ...
      const mockWhere2 = vi
        .fn()
        .mockResolvedValue([{ area: { name: "Area 1" } }]);
      ...
      const mockWhere3 = vi
        .fn()
        .mockResolvedValue([{ branch: { name: "Branch 1" } }]);
  ...
  describe("getUserBranches", () => {
    it("should return user branches", async () => {
      const mockBranches = [{ branch: { id: "1" } }, { branch: { id: "2" } }];
  ...
  describe("recordUserLogin", () => {
    it("should record successful login", async () => {
      ...
      const result = await userQueries.recordUserLogin(
        db,
        "1",
        "127.0.0.1",
        "Mozilla",
