...
describe("Sync Jobs Routes", () => {
  ...
  describe("GET /api/sync/jobs", () => {
    ...
    it("should return 500 on error", async () => {
      vi.mocked(hasPermission).mockResolvedValue(true);
      vi.mocked(getRecentSyncJobs).mockRejectedValue(
  ...
  describe("POST /api/sync/jobs", () => {
    ...
    it("should return 500 on error", async () => {
      vi.mocked(hasPermission).mockResolvedValue(true);
      vi.mocked(snowflakeSyncService.sync).mockRejectedValue(
  ...
  describe("POST /api/sync/jobs/preview", () => {
    it("should return snowflake preview with permission", async () => {
      ...
      vi.mocked(hasPermission).mockResolvedValue(true);
      vi.mocked(snowflakeSyncService.fetchPreview).mockResolvedValue(
        mockPreview as any,
      );
      ...
      expect(snowflakeSyncService.fetchPreview).toHaveBeenCalledWith(
        ["branch1"],
        100,
      );
    });
    ...
    it("should return 500 on error", async () => {
      vi.mocked(hasPermission).mockResolvedValue(true);
      vi.mocked(snowflakeSyncService.fetchPreview).mockRejectedValue(
