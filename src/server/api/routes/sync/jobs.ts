...
syncJobsRoutes.get(
  ...
  async (c) => {
    ...
    try {
      // Check permission
      const hasReadPermission = await hasPermission(userId, orgId, 'sync', 'execute')
      ...
      return c.json(
        {
          success: false,
          error: {
            message: error instanceof Error ? error.message : 'Failed to retrieve sync jobs',
          },
        },
        500
      )
      ...
      return c.json(
        {
          success: false,
          error: {
            message: error instanceof Error ? error.message : 'Failed to retrieve sync job details',
          },
        },
        500
      )
...
syncJobsRoutes.post(
  ...
  async (c) => {
    ...
    try {
      // Check permission
      const hasExecutePermission = await hasPermission(userId, orgId, 'sync', 'execute')
      ...
      return c.json(
        {
          success: false,
          error: {
            message: error instanceof Error ? error.message : 'Failed to execute sync job',
          },
        },
        500
      )
  ...
  async (c) => {
    ...
    try {
      ...
      if (type === 'snowflake') {
        ...
        previewResult = await snowflakeSyncService.fetchPreview(options?.branchCodes, 100)
      } else if (type === 'nextbank') {
        // NextBank sync not implemented yet
        return c.json(
          {
      ...
      return c.json(
        {
          success: false,
          error: {
            message: error instanceof Error ? error.message : 'Failed to preview sync job',
          },
        },
        500
      )
