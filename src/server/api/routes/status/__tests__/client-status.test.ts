...
describe("Client Status Routes", () => {
  ...
  describe("GET /api/status/:clientId", () => {
    it("should return client status with permission", async () => {
      ...
      const response = await app.request(
        "/client-1?periodType=monthly&periodYear=2024&periodMonth=1",
      );
    ...
    it("should return 403 when user lacks permission", async () => {
      ...
      const response = await app.request(
        "/client-1?periodType=monthly&periodYear=2024",
      );
      const json = await response.json();
    ...
    it("should return 500 on error", async () => {
      vi.mocked(hasPermission).mockResolvedValue(true);
      vi.mocked(getClientCurrentStatus).mockRejectedValue(
  ...
  describe("GET /api/status/:clientId/history", () => {
    ...
    it("should return 500 on error", async () => {
      vi.mocked(hasPermission).mockResolvedValue(true);
      vi.mocked(getClientStatusHistory).mockRejectedValue(
        new Error("Database error"),
      );
