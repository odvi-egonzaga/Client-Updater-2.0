...
describe('User Sessions Routes', () => {
  ...
  describe('DELETE /api/users/:id/sessions/:sessionId', () => {
    it('should revoke a single session', async () => {
      ...
      expect(revokeSession).toHaveBeenCalledWith(db, '1', 'Session revoked by admin')
    })
  ...
  describe('POST /api/users/:id/sessions/revoke-all', () => {
    it('should revoke all user sessions', async () => {
      ...
      expect(revokeAllUserSessions).toHaveBeenCalledWith(db, '1', 'All sessions revoked by admin')
    })

    it('should return 500 on error', async () => {
      vi.mocked(revokeAllUserSessions).mockRejectedValue(new Error('Database error'))
