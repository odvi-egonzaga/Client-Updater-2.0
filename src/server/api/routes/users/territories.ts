    ...
    return c.json(
      {
        success: false,
        error: {
          message: error instanceof Error ? error.message : 'Failed to retrieve user territories',
        },
      },
      500
    )
...
userTerritoriesRoutes.put('/:id/territories', zValidator('json', setTerritoriesSchema), async (c) => {
  ...
  try {
    ...
    await Promise.all([
      currentAreaIds.length > 0 ? removeAreasFromUser(db, userId, currentAreaIds) : Promise.resolve([]),
      currentBranchIds.length > 0 ? removeBranchesFromUser(db, userId, currentBranchIds) : Promise.resolve([]),
    ])

    // Add new territories
    const [newAreas, newBranches] = await Promise.all([
      areaIds.length > 0 ? assignAreasToUser(db, userId, areaIds) : Promise.resolve([]),
      branchIds.length > 0 ? assignBranchesToUser(db, userId, branchIds) : Promise.resolve([]),
    ])
    ...
    return c.json(
      {
        success: false,
        error: {
          message: error instanceof Error ? error.message : 'Failed to set user territories',
        },
      },
      500
    )
