...
healthRoutes.get("/all", async (c) => {
  ...
  const checks = [
    ...
    {
      name: "Database Write",
      url: `${baseUrl}/api/health/database/write`,
      method: "POST" as const,
    },
    { name: "Database Read", url: `${baseUrl}/api/health/database/read` },
    {
      name: "Database Delete",
      url: `${baseUrl}/api/health/database/delete`,
      method: "DELETE" as const,
    },
    {
      name: "Storage Upload",
      url: `${baseUrl}/api/health/storage/upload`,
      method: "POST" as const,
    },
    { name: "Storage Download", url: `${baseUrl}/api/health/storage/download` },
    {
      name: "Storage Delete",
      url: `${baseUrl}/api/health/storage/delete`,
      method: "DELETE" as const,
    },
    ...
    { name: "Edge Auth", url: `${baseUrl}/api/health/edge/auth` },
    {
      ...
      url: `${baseUrl}/api/health/snowflake/connect`,
    },
  ...
  const services = {
    clerk: results.filter(
      (r) => r.status === "fulfilled" && r.value.name.includes("Clerk"),
    ),
    database: results.filter(
      (r) => r.status === "fulfilled" && r.value.name.includes("Database"),
    ),
    storage: results.filter(
      (r) => r.status === "fulfilled" && r.value.name.includes("Storage"),
    ),
    edge: results.filter(
      (r) => r.status === "fulfilled" && r.value.name.includes("Edge"),
    ),
    snowflake: results.filter(
      (r) => r.status === "fulfilled" && r.value.name.includes("Snowflake"),
    ),
    nextbank: results.filter(
      (r) => r.status === "fulfilled" && r.value.name.includes("NextBank"),
    ),
    awsS3: results.filter(
      (r) => r.status === "fulfilled" && r.value.name.includes("AWS S3"),
    ),
  };
  ...
  const hasErrors = allResults.some(
    (r) => r && (r.status === "error" || r.httpStatus >= 400),
  );
