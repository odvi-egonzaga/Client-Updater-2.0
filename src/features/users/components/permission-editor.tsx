...
export function PermissionEditor({
  ...
  const [scopes, setScopes] = useState<Record<string, 'self' | 'all' | 'team' | 'branch' | 'area'>>(
  ...
  const handleScopeChange = (permissionId: string, scope: 'self' | 'all' | 'team' | 'branch' | 'area') => {
    setScopes((prev) => ({
      ...prev,
      [permissionId]: scope,
    }))
    ...
    <Card>
      ...
      <CardContent>
                        ...
                        <div className="flex items-start justify-between gap-4">
                          ...
                          {isSelected && (
                            <div className="flex items-center gap-2">
                              <label htmlFor={`scope-${permission.id}`} className="text-sm font-medium">
                                Scope:
                              </label>
                              <select
                                ...
                                onChange={(e) =>
                                  handleScopeChange(
                                    permission.id,
                                    e.target.value as 'self' | 'all' | 'team' | 'branch' | 'area'
                                  )
                                }
        ...
        <div className="mt-6 flex justify-end gap-2">
          ...
          <Button onClick={handleSave} disabled={isSaving || selectedPermissions.size === 0}>
            {isSaving ? (
              <>
