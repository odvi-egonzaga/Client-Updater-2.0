...
export function HealthDashboard() {
  const [individualChecks, setIndividualChecks] = useState<
    Record<string, IndividualCheckResult>
  >({});
  ...
  const totalHealthChecks = SERVICES.reduce(
    ...
    try {
      ...
      if (endpoint.startsWith("client:")) {
        ...
        const response = await fetch(`/api/health${endpoint}`, {
          ...
          ...(endpoint === "/database/delete" || endpoint === "/storage/delete"
            ? { method: "DELETE" }
            : {}),
        });
  ...
  const runAllChecks = async () => {
    ...
    const promises = SERVICES.flatMap((service) =>
      service.checks.map((check) =>
        runIndividualCheck(service.name, check.name, check.endpoint),
      ),
    );
  ...
  const getServiceStatus = (serviceName: string): HealthStatus => {
    ...
    if (
      checks.some(
        (c) => c.status === "error" || (c.httpStatus && c.httpStatus >= 400),
      )
    )
      return "error";
    ...
    <div className="space-y-6">
        ...
        <Collapsible defaultOpen>
          <CollapsibleTrigger className="group w-full">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                <Badge
                  className={statusColors[overallStatus]}
                  variant="outline"
                >
      ...
      <div className="space-y-4">
        {SERVICES.map((service) => {
          ...
          const isServiceLoading = Array.from(loadingChecks).some((key) =>
                  ...
                  <div className="mb-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="text-xl">
                        {statusIcons[serviceStatus]}
                      </div>
                      <div className="text-left">
                        <h3 className="font-semibold">{service.name}</h3>
                        <p className="text-muted-foreground text-sm">
                          {service.checks.length} check
                          {(service.checks.length as number) !== 1 ? "s" : ""}
                        </p>
                      </div>
                      <Button
                        size="sm"
                    ...
                    <div className="flex items-center gap-4">
                      <Badge
                        className={statusColors[serviceStatus]}
                        variant="outline"
                      >
                ...
                <CollapsibleContent>
                  <div className="space-y-3">
                    {service.checks.map((check) => {
                      ...
                      const checkStatus: HealthStatus =
                        individualResult?.status ?? "pending";
                      ...
                      return (
                        ...
                        >
                          <Collapsible defaultOpen={!!individualResult}>
                            ...
                            <CollapsibleContent>
                              {individualResult && (
                                <div className="bg-background mt-2 rounded border p-3">
                                  <div className="mb-2 flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                      {statusIcons[individualResult.status]}
                                      <Badge
                                        className={
                                          statusColors[individualResult.status]
                                        }
                                        variant="outline"
                                      >
                                  ...
                                  {individualResult.data && (
                                    <div className="mt-2">
                                      <pre className="bg-muted max-h-[200px] overflow-auto rounded p-2 text-xs whitespace-pre-wrap">
                                        {JSON.stringify(
                                          individualResult.data,
                                          null,
                                          2,
