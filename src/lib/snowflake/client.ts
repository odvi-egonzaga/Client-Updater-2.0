...
function formatPrivateKey(key: string | undefined): string | undefined {
  ...
  if (!formattedKey.includes("BEGIN PRIVATE KEY")) {
    console.warn(
      'Snowflake Private Key may be invalid or missing PKCS#8 header (expected "-----BEGIN PRIVATE KEY-----")',
    );
  }

  if (formattedKey.includes("BEGIN RSA PRIVATE KEY")) {
    console.warn(
      'Snowflake Private Key appears to be PKCS#1 (RSA). Snowflake requires PKCS#8. Use "openssl pkcs8 -topk8..." to convert.',
    );
  }
