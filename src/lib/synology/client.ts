...
class SynologyClient {
  ...
  private async fetchWithAgent(url: string, options: RequestInit = {}) {
    ...
    try {
      ...
      if (
        ...
        err.cause?.message?.includes("certificate") ||
        err.cause?.code === "DEPTH_ZERO_SELF_SIGNED_CERT"
      ) {
        console.warn(
          "[Synology] Certificate error detected. Retrying with permissive fetch...",
        );
  ...
  async ping(): Promise<{
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Synology API error: ${response.status} ${response.statusText}`,
        );
      }
  ...
  private async login(): Promise<string> {
    ...
    try {
      const response = await this.fetchWithAgent(
        `${loginUrl}?${params.toString()}`,
      );

      if (!response.ok) {
        throw new Error(
          `Synology Auth error: ${response.status} ${response.statusText}`,
        );
      }
