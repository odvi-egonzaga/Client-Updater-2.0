...
export async function initializePeriod(
  ...
  try {
      ...
      .where(
        and(
          ...
          periodMonth !== undefined
            ? eq(clientPeriodStatus.periodMonth, periodMonth)
            : undefined,
          periodQuarter !== undefined
            ? eq(clientPeriodStatus.periodQuarter, periodQuarter)
            : undefined,
        ),
      );
    ...
    const records: CreateClientPeriodStatusInput[] = clientsToCreate.map(
      (client) => ({
        clientId: client.id,
        periodType,
        periodYear,
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
      ...
      for (let i = 0; i < batch.length; i++) {
        ...
        try {
          ...
          logger.error(
            "Failed to create period status in batch",
            error as Error,
            {
