...
describe('Health Check Types', () => {
  describe('HealthStatus', () => {
    it('should accept valid health status values', () => {
      const statuses: HealthStatus[] = ['healthy', 'unhealthy', 'error', 'warning', 'pending']
  ...
  describe('EdgeFunctionHealthResult', () => {
    it('should create a valid edge function health result', () => {
      ...
      expect(result.endpoint).toBe('https://example.com/functions/v1/test-function')
    })
  ...
  describe('EdgeFunctionCheckConfig', () => {
    ...
    it('should include optional fields', () => {
      ...
      expect(config.endpoint).toBe('https://example.com/functions/v1/test-function')
  ...
  describe('HealthCheckError', () => {
    ...
    it('should include optional original error', () => {
      ...
      const error = new HealthCheckError('Test error', 'test-component', originalError)
  ...
  describe('EdgeFunctionHealthError', () => {
    ...
    it('should include optional http status', () => {
      const error = new EdgeFunctionHealthError('Test error', 'test-function', 404)
      expect(error.httpStatus).toBe(404)
    })

    it('should include optional original error', () => {
      const originalError = new Error('Original error')
      const error = new EdgeFunctionHealthError('Test error', 'test-function', 404, originalError)
      expect(error.originalError).toBe(originalError)
    })
  })
  ...
  describe('DatabaseHealthError', () => {
    it('should create a valid database health error', () => {
      const error = new DatabaseHealthError('Test error', 'write', 'test-table')
      expect(error.message).toBe('Test error')
    ...
    it('should include optional original error', () => {
      const originalError = new Error('Original error')
      const error = new DatabaseHealthError('Test error', 'write', 'test-table', originalError)
      expect(error.originalError).toBe(originalError)
    })
  })
})
