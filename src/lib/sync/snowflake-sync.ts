...
export async function buildLookupCache(): Promise<LookupCache> {
  try {
    const [pensionTypesData, pensionerTypesData, productsData, branchesData, parStatusesData, accountTypesData] =
      await Promise.all([
...
export function transformRecord(
  ...
  return {
    ...
    birthDate: snowflakeRecord.BIRTH_DATE instanceof Date
    ...
    pensionTypeId: lookupCache.pensionTypes.get(snowflakeRecord.PENSION_TYPE_CODE),
    pensionerTypeId: lookupCache.pensionerTypes.get(snowflakeRecord.PENSIONER_TYPE_CODE),
    ...
    accountTypeId: lookupCache.accountTypes.get(snowflakeRecord.ACCOUNT_TYPE_CODE),
...
export async function syncClientsFromSnowflake(
  ...
  try {
    ...
    const snowflakeClients = await fetchClientsFromSnowflake(options.branchCodes)
