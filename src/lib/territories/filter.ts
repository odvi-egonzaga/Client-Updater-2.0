...
export async function getUserBranchIds(
  ...
  try {
    ...
    try {
      ...
      logger.error('Database fallback failed for user branches', dbError as Error, {
        action: 'get_user_branch_ids',
        userId,
        companyId,
      })
...
export async function getUserBranchFilter(
  ...
  try {
    ...
    const hasAllAccess = await hasPermission(userId, companyId, 'clients', 'read')
...
export async function invalidateAllUserBranchCache(): Promise<void> {
  try {
    ...
    logger.error('Failed to invalidate all user branch caches', error as Error, {
      action: 'invalidate_all_user_branch_cache',
    })
